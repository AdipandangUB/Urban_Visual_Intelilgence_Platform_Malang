...
def _extract_place_in_cell_photos(file_bytes: bytes, foto_col_name: str):
    """Cari foto yang disisipkan lewat fitur Excel 365 "Place in Cell" /
    "Picture in Cell" (bukan gambar melayang di atas sel, dan bukan pula
    teks URL) pada kolom FOTO.

    Fitur ini menyimpan gambar sebagai *rich value* ("Data Type" khusus
    Excel: xl/richData/*.xml + xl/metadata.xml), bukan sebagai objek gambar
    biasa (`xl/drawings`) — sehingga tidak terdeteksi oleh
    `ws._images` openpyxl sama sekali. Saat file dibuka alat lain (pandas,
    Google Sheets, dsb.) sel yang berisi gambar jenis ini biasanya tampil
    sebagai nilai error "#VALUE!", walau gambarnya sendiri tetap tersimpan
    utuh di dalam file .xlsx (di `xl/media/`) dan bisa diambil manual lewat
    rantai relasi: sel (atribut `vm`) -> `xl/metadata.xml` -> indeks rich
    value -> `xl/richData/rdrichvalue.xml` -> indeks relasi ->
    `xl/richData/richValueRel.xml` (+ file `.rels`-nya) -> path gambar
    aktual di `xl/media/`.

    Return: dict {pandas_row_index: data_uri}. Dict kosong kalau file ini
    tidak memakai fitur rich-value picture-in-cell, atau kalau terjadi
    error apa pun saat parsing (upload tetap lanjut tanpa foto jenis ini).
    """
    if not _EMBEDDED_IMAGE_SUPPORT:
        return {}
    try:
        with zipfile.ZipFile(io.BytesIO(file_bytes)) as z:
            ...
            if not required.issubset(names):
                return {}  # file ini tidak pakai fitur picture-in-cell
            ...
            # 1) valueMetadata: index (0-based, urutan dokumen) -> indeks rv
            meta_root = ET.fromstring(z.read("xl/metadata.xml"))
            value_meta = meta_root.find(f"{ns_main}valueMetadata")
            rv_index_by_meta = []
            if value_meta is not None:
                for bk in value_meta.findall(f"{ns_main}bk"):
                    rc = bk.find(f"{ns_main}rc")
                    if rc is not None and rc.get("t") == "1":
                        rv_index_by_meta.append(int(rc.get("v")))
                    else:
                        rv_index_by_meta.append(None)

            # 2) rdrichvalue.xml: index rv (0-based) -> indeks relasi gambar
            #    (nilai <v> PERTAMA pada tiap <rv>, sesuai struktur
            #    "_localImage" di rdrichvaluestructure.xml: field pertama =
            #    _rvRel:LocalImageIdentifier).
            rv_root = ET.fromstring(z.read("xl/richData/rdrichvalue.xml"))
            rel_index_by_rv = []
            for rv in rv_root.findall(f"{ns_rd}rv"):
                v_els = rv.findall(f"{ns_rd}v")
                rel_index_by_rv.append(int(v_els[0].text) if v_els else None)

            # 3) richValueRel.xml: indeks relasi (0-based, urutan dokumen)
            #    -> r:id
            rvrel_root = ET.fromstring(z.read("xl/richData/richValueRel.xml"))
            rids = [
                rel.get(f"{{{ns_rel}}}id")
                for rel in rvrel_root.findall(
                    "{http://schemas.microsoft.com/office/spreadsheetml/2022/richvaluerel}rel"
                )
            ]

            # 4) richValueRel.xml.rels: r:id -> path gambar aktual (relatif
            #    terhadap folder xl/richData/)
            ...
            def image_bytes_for_vm(vm_1indexed):
                """vm (atribut sel, 1-indexed) -> bytes gambar, atau None."""
                meta_idx = vm_1indexed - 1
                if meta_idx < 0 or meta_idx >= len(rv_index_by_meta):
                    return None
                rv_idx = rv_index_by_meta[meta_idx]
                if rv_idx is None or rv_idx >= len(rel_index_by_rv):
                    return None
                rel_idx = rel_index_by_rv[rv_idx]
                if rel_idx is None or rel_idx >= len(rids):
                    return None
                rid = rids[rel_idx]
                ...
                img_path = posixpath.normpath(
                    posixpath.join("xl/richData", target)
                )
                if img_path not in names:
                    return None
                return z.read(img_path)

            # 5) Cari file worksheet pertama (sama seperti yang dibaca
            #    pandas.read_excel secara default) lewat workbook.xml +
            #    relasinya, lalu ambil semua sel dengan atribut `vm`.
            wb_root = ET.fromstring(z.read("xl/workbook.xml"))
            first_sheet = wb_root.find(f"{ns_main}sheets/{ns_main}sheet")
            if first_sheet is None:
                return {}
            sheet_rid = first_sheet.get(f"{{{ns_rel}}}id")
            wb_rels_root = ET.fromstring(z.read("xl/_rels/workbook.xml.rels"))
            sheet_target = None
            for rel in wb_rels_root.findall(f"{ns_pkgrel}Relationship"):
            ...
            foto_col_idx = None
            # Header ada di baris 1 -> cari kolom "foto" dulu dari nilai sel
            # (dibaca langsung dari XML, tanpa openpyxl, supaya konsisten
            # walau workbook berisi rich value yang bisa bikin openpyxl
            # "tersandung").
            ...
            for row_el in sheet_data.findall(f"{ns_main}row"):
                for c in row_el.findall(f"{ns_main}c"):
                    ...
                    if row_num == 1:
                        # Baris header -> cari posisi kolom "foto"
                        if foto_col_idx is None:
                            text = cell_text(c)
                            if text and text.strip().lower() == foto_col_name.strip().lower():
                                foto_col_idx = col_idx
                        continue

                    vm = c.get("vm")
                    if vm is None or foto_col_idx is None or col_idx != foto_col_idx:
                        continue

                    raw_bytes = image_bytes_for_vm(int(vm))
                    if not raw_bytes:
                        continue
                    data_uri = _encode_embedded_image(raw_bytes)
                    if not data_uri:
                        continue
                    pandas_index = row_num - 2  # excel row 2 (1-idx) = pandas index 0
                    if pandas_index >= 0:
                        result[pandas_index] = data_uri
            return result
    except Exception:
...
def parse_uploaded_corridor(file_obj, filename: str):
    ...
    if is_excel and file_bytes and foto_col_name is not None:
        if _EMBEDDED_IMAGE_SUPPORT:
            # Coba dua mekanisme penyisipan foto yang umum dipakai di Excel:
            # (a) gambar biasa yang di-insert/drag melayang di atas sel
            #     ("floating drawing" klasik), dan
            # (b) fitur baru Excel 365 "Place in Cell" / picture-in-cell,
            #     yang tersimpan sebagai rich-value dan sering tampil
            #     sebagai "#VALUE!" kalau file dibuka di luar Excel 365.
            # Baris yang sama tidak mungkin punya dua-duanya, jadi aman
            # digabung begitu saja.
            embedded_photos.update(
                _extract_embedded_photos_by_pandas_index(file_bytes, foto_col_name)
            )
            embedded_photos.update(
                _extract_place_in_cell_photos(file_bytes, foto_col_name)
            )
        else:
            warnings.append(
