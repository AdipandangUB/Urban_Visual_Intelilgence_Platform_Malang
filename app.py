...
# Titik pusat (center of view) khusus untuk koridor tertentu, sesuai
# permintaan: tampilan awal peta koridor ini harus berpusat tepat di
# koordinat berikut (bukan sekadar rata-rata koordinat node-nya).
# Key dinormalisasi (huruf kecil, tanpa spasi/simbol) supaya tetap cocok
# walau penulisan nama koridor sedikit berbeda (mis. "Kayutangan-Tugu"
# vs "Kayutangan – Tugu").
CORRIDOR_VIEW_CENTERS = {
    "kayutangan1": (-7.978294, 112.629494),
    "kayutangantugu": (-7.976644, 112.631425),
}


def _corridor_view_center(corridor_name):
    """Ambil override center-of-view untuk sebuah koridor, jika ada."""
    key = re.sub(r"[^a-z0-9]", "", str(corridor_name or "").lower())
    return CORRIDOR_VIEW_CENTERS.get(key)
...
with tab_map:
    ...
    with left:
        ...
        if valid.empty:
            ...
            n_nodes = len(valid)
            lat_min, lat_max = valid["lat"].min(), valid["lat"].max()
            lon_min, lon_max = valid["lon"].min(), valid["lon"].max()

            # Untuk koridor tertentu (Kayutangan 1, Kayutangan-Tugu), titik
            # pusat tampilan awal sudah ditentukan secara eksplisit. Koridor
            # lain tetap memakai rata-rata koordinat node-nya seperti semula.
            view_center_override = _corridor_view_center(selected_corridor)
            if view_center_override:
                center = list(view_center_override)
            else:
                center = [valid["lat"].mean(), valid["lon"].mean()]

            # Zoom awal dipakai hanya sebagai fallback (mis. koridor dengan 1
            # titik saja). Untuk koridor dengan banyak titik, tampilan awal
            # di-fit ke bounding box datanya sendiri (lihat fit_bounds di
            # bawah) supaya setiap koridor -- baik yang memanjang vertikal
            # (mis. Kayutangan 1) maupun horizontal/diagonal (mis.
            # Kayutangan-Tugu) -- langsung tampil rapi memenuhi frame peta,
            # bukan selalu memakai zoom tetap (17) yang membuat titik-titik
            # yang berdekatan menumpuk & label angkanya saling tindih.
            ...
            if show_nodes:
                # Node fotogenik pada satu koridor biasanya berjarak sangat
                # dekat satu sama lain (setiap beberapa meter), sehingga
                # marker & label skor mudah saling tumpuk. Ukuran marker dan
                # font label dibuat menyesuaikan jumlah titik: makin banyak
                # titik pada koridor, makin kecil ukurannya.
                if n_nodes > 40:
                    marker_radius, font_px = 5, 8
                elif n_nodes > 20:
                    marker_radius, font_px = 6, 9
                else:
                    marker_radius, font_px = 9, 11
                ...
                for i, (_, row) in enumerate(valid.iterrows()):
                    ...
                    # Label diselang-seling naik/turun (zig-zag) agar dua
                    # titik yang berdekatan tidak selalu saling menimpa teks.
                    offset_y = -10 if i % 2 == 0 else 12
                    folium.map.Marker(
                        [row["lat"], row["lon"]],
                        icon=folium.DivIcon(html=f"""
                            <div style="font-size:{font_px}px;font-weight:700;color:white;
                                        text-shadow:1px 1px 2px #000;white-space:nowrap;
                                        transform:translate(8px,{offset_y}px);">
                                {row['UVI']:.2f}
                            </div>""")
                    ).add_to(fmap)

            # Tampilan awal (initial view) di-fit otomatis ke cakupan data
            # koridor terpilih, bukan memakai zoom tetap. Ini yang membuat
            # peta koridor pendek/berdekatan (mis. Kayutangan 1, memanjang
            # utara-selatan) maupun koridor lebih panjang & diagonal (mis.
            # Kayutangan-Tugu) sama-sama langsung terlihat penuh & proporsional
            # begitu peta dibuka, tanpa perlu zoom/geser manual.
            if lat_min == lat_max and lon_min == lon_max:
                fmap.location = center
                fmap.zoom_start = 17
            elif view_center_override:
                # Koridor dengan center-of-view tetap: bounds dibuat SIMETRIS
                # terhadap titik center tersebut (setengah-lebar/tinggi =
                # jarak terjauh node ke titik center), supaya setelah
                # fit_bounds, titik pusat peta tetap persis di koordinat yang
                # diminta -- bukan bergeser ke rata-rata koordinat node.
                lat0, lon0 = view_center_override
                half_lat = max(abs(lat_max - lat0), abs(lat_min - lat0), 0.0006)
                half_lon = max(abs(lon_max - lon0), abs(lon_min - lon0), 0.0006)
                pad_lat = half_lat * 0.15
                pad_lon = half_lon * 0.15
                fmap.fit_bounds([
                    [lat0 - half_lat - pad_lat, lon0 - half_lon - pad_lon],
                    [lat0 + half_lat + pad_lat, lon0 + half_lon + pad_lon],
                ])
            else:
                pad_lat = max((lat_max - lat_min) * 0.15, 0.0006)
                pad_lon = max((lon_max - lon_min) * 0.15, 0.0006)
                fmap.fit_bounds([
