...
""")
        ...
        # Versi Markdown tetap disediakan sebagai format editable/backup.
        with st.expander("Versi editable (Markdown)", expanded=False):
            pb_text = policy_brief_markdown(corridor_pb, indicator_pb, pb_scope)
            st.download_button(
                "⬇️ Unduh Policy Brief (.md)",
                data=pb_text.encode("utf-8"),
                file_name=f"policy_brief_UVI_Malang_{datetime.now():%Y%m%d}.md",
                mime="text/markdown",
                key="download_policy_brief_md",
            )
