# -*- coding: utf-8 -*-
"""
UVIP Malang — Platform Intelijen Visual Perkotaan
=================================================
...
- Indeks Visual Perkotaan (UVI)      -> indeks komposit kualitas visual per titik/segmen
- Citra Jalan                       -> data citra jalan (Google Street View / survei 360)
- Kecerdasan Buatan                 -> hasil deteksi elemen visual (segmentasi semantik)
- Ruang Terbuka Kota                -> unit analisis = koridor jalan / ruang terbuka
- Kota Cerdas                       -> dashboard WebGIS untuk pemantauan & simulasi kebijakan
...
# build_node_popup_html diimpor secara defensif: kalau uvip_core.py yang
# ter-deploy di server masih versi lama (belum berisi fungsi ini — biasanya
# karena file itu belum ikut di-push/redeploy), app TIDAK boleh crash.
# Sebagai gantinya dipakai salinan cadangan yang identik di bawah ini.
try:
    ...
    def build_node_popup_html(row, header_color: str) -> str:
        ...
        koridor = html.escape(str(row.get("corridor", "")))
        sisi = html.escape(str(row.get("side", "")))
...
# ----------------------------------------------------------------------------
# 0.1 FUNGSI UNTUK GAMBAR LATAR BELAKANG
# ----------------------------------------------------------------------------
...
DEFAULT_BASEMAP = "Peta Dasar OpenStreetMap"
...
st.sidebar.caption(
    "**P**latform **I**ntelijen **V**isual **P**erkotaan — piranti WebGIS Analytics "
    "*kota cerdas* untuk menilai & mensimulasikan kualitas visual ruang terbuka "
...
if demo_used:
    st.sidebar.warning(
        ...
        ". Pastikan Google Sheet sudah dibagikan sebagai **'Siapa pun dengan "
        "tautan — Pemirsa'** agar data riil dapat dimuat.",
        icon="⚠️",
    )
...
st.sidebar.markdown("---")
st.sidebar.caption(
    "Sumber data: hasil deteksi AI (computer vision) atas citra "
    "tingkat-jalan koridor Kota Malang, diproses tim Peneliti UVIP "
    "(Platform Intelijen Visual Perkotaan) Malang - Universitas Brawijaya. "
)
...
if header_bg_set:
    st.markdown(
        ...
        <div class="header-with-bg">
            ...
            <p>
                Berbasis AI untuk <b>Kota Cerdas Malang</b> — adaptasi
                <i>Indeks Visual Perkotaan (UVI)</i> dari citra <i>tingkat-jalan</i>,
...
tab_map, tab_indicators, tab_sim, tab_compare, tab_about, tab_data = st.tabs(
    ["🗺️ Peta Titik Panas UVI", "📊 Indikator Visual", "🧮 Simulasi Skenario",
     "📈 Perbandingan Koridor", "📄 Tentang & Metodologi", "📥 Data & Unduh"]
)

# ----------------------------------------------------------------------------
# TAB 1 — PETA TITIK PANAS (heatmap + node fotogenik, mengacu pada visual acuan)
# ----------------------------------------------------------------------------
with tab_map:
    ...
    with right:
        ...
        basemap_name = st.selectbox("Peta Dasar", list(BASEMAPS.keys()),
                                     index=list(BASEMAPS.keys()).index(DEFAULT_BASEMAP))
        show_nodes = st.checkbox("Tampilkan titik fotogenik + skor UVI", value=True)
        show_heat = st.checkbox("Tampilkan peta panas kepadatan UVI", value=True)
    ...
    st.caption(
        "Peta ini mensimulasikan luaran 'HASIL & LUARAN: PETA TITIK PANAS PVI' pada "
        "proposal — peta panas kualitas visual dengan titik fotogenik yang menampilkan "
...
with tab_indicators:
    ...
    with c2:
        ...
        pick = st.selectbox("Pilih titik untuk melihat breakdown kelas visual", node_options)
        ...
        if pie_data:
            ...
            st.info("Tidak ada data breakdown kelas visual untuk titik ini.")

    st.markdown("#### Distribusi Skor UVI per Indikator (seluruh titik terpilih)")
    box_rows = []
    for ind in INDICATORS:
        col = f"uvi::{ind}"
...
with tab_sim:
    st.markdown(
        ...
        "Ubah proporsi elemen visual pada sebuah titik (mis. menambah vegetasi, "
        "mengurangi papan reklame/kendaraan) untuk melihat dampaknya terhadap skor UVI — "
        "mensimulasikan modul *'perubahan parameter desain'* pada proposal."
    )

    node_options2 = df["kode"] + " — " + df["corridor"]
    pick2 = st.selectbox("Pilih titik yang akan disimulasikan", node_options2, key="sim_node")
...
with tab_about:
    ...
    st.markdown(
        """
        **UVIP Malang** (*Platform Intelijen Visual Perkotaan*) adalah piranti WebGIS Analytics
        *kota cerdas* yang dikembangkan pada Penelitian Terapan
        *"Pengembangan Model Sistem Simulasi Visual Digital Ruang Terbuka Perkotaan
        Berbasis AI untuk Kota Cerdas Malang"* 
        """
    )
    
    st.markdown(
        """
        **Kata kunci konteks:** Indeks Visual Perkotaan · Citra Tingkat-Jalan ·
        Kecerdasan Buatan · Ruang Terbuka Kota · Kota Cerdas
        """
    )
    ...
    with col1:
        ...
        st.markdown(
            """
            1. **Pengumpulan citra tingkat-jalan** pada segmen koridor ruang terbuka
               (mis. Kayutangan, Tugu, Lafayette–PLN, Alun-Alun Merdeka).
            2. **Deteksi elemen visual berbasis AI** (segmentasi semantik) menghasilkan
               proporsi kelas: *Tanah, Bangunan, Rambu Lalu Lintas, Vegetasi, Langit,
               Manusia, Kendaraan 4/2 roda*.
            3. **Perhitungan 8 indikator visual komposit**: Visibilitas Bangunan,
               Cakupan Vegetasi, Keterbukaan Langit, Aksesibilitas Lahan, Aktivitas Manusia,
               Intensitas Kendaraan, Infrastruktur Lalu Lintas, Dominasi Warisan Budaya.
            4. **Pembobotan & agregasi** menjadi **Indeks Visual Perkotaan (UVI)** — pada
               platform ini bobot dapat diubah interaktif untuk mensimulasikan skenario
               preferensi publik/kebijakan.
            5. **Visualisasi & simulasi** pada dashboard: peta titik panas, radar indikator,
    ...
    with col2:
        ...
        st.markdown(
            """
            - Bobot UVI pada prototipe ini bersifat *dapat-diatur-pengguna* untuk kebutuhan
              demonstrasi; kalibrasi final memerlukan survei persepsi publik (≥200
              responden) dan analisis statistik lanjutan.
            - Data ditarik langsung dari Google Sheets hasil kerja tim (bukan jalur-pipa
              model AI *end-to-end*).
            - Simulasi skenario pada Tab 3 bekerja pada tingkat indeks, bukan pada tingkat
              regenerasi citra.
            """
        )
    ...
    researchers = [
        {
            ...
            "expertise": "Ahli *Bangunan Digital 3D* | *Manajemen Aset Digital*",
            "role": "Ketua Tim Peneliti"
        },
        {
            "name": "Dr. Adipandang Yudono",
            "image": "images/Adipandang Yudono.jpeg",
            "expertise": "Ahli *Ilmu Data Spasial* | *Pemrograman SIG* | *Kota Cerdas* | *GeoAI* ",
            "role": "Anggota Tim Peneliti"
        },
        {
            "name": "Dr. Herman Tolle",
            "image": "images/herman tolle.png",
            "expertise": "Ahli Teknologi Informasi | Sistem Cerdas | *Interaksi Manusia-Komputer*",
            "role": "Anggota Tim Peneliti"
        },
        {
        ...
        },
        {
            ...
            "expertise": "Ahli Kecerdasan Buatan | *Penglihatan Komputer* | *Ilmu Data*",
            "role": "Anggota Tim Peneliti"
        }
    ]
