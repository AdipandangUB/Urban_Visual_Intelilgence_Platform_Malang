...
PHOTO_SOURCES = {
    "385636298": {
        ...
        "url": "https://docs.google.com/spreadsheets/d/1XrgdsW7IVULMQ3LIiEocPAbuFUGBkAnaPBYO0N2gxMA/edit?gid=385636298#gid=385636298",
    },
    "1869212551": {
        "name": "Kayutangan 1",
        "url": "https://docs.google.com/spreadsheets/d/1XrgdsW7IVULMQ3LIiEocPAbuFUGBkAnaPBYO0N2gxMA/edit?gid=1869212551#gid=1869212551",
    },
    "891123454": {
        "name": "Lavalette-PLN",
        "url": "https://docs.google.com/spreadsheets/d/1XrgdsW7IVULMQ3LIiEocPAbuFUGBkAnaPBYO0N2gxMA/edit?gid=891123454#gid=891123454",
    },
    "446487299": {
        "name": "Kayutangan-Tugu",
        "url": "https://docs.google.com/spreadsheets/d/1XrgdsW7IVULMQ3LIiEocPAbuFUGBkAnaPBYO0N2gxMA/edit?gid=446487299#gid=446487299",
    },
    "1062597004": {
        "name": "Tugu",
        "url": "https://docs.google.com/spreadsheets/d/1XrgdsW7IVULMQ3LIiEocPAbuFUGBkAnaPBYO0N2gxMA/edit?gid=1062597004#gid=1062597004",
    },
}
...
def _find_photo_column(columns):
    """Cari kolom foto secara fleksibel tanpa mengunci nama kolom tertentu."""
    preferred = []
    for col in columns:
...
def _attach_photo_urls(data, gid):
    ...
    if {'lat', 'lon'}.issubset(out.columns):
        ...
        if raw_lat_col and raw_lon_col:
            ...
            for _, rr in raw.iterrows():
                try:
                    la = round(float(rr[raw_lat_col]), 6)
                    lo = round(float(rr[raw_lon_col]), 6)
                    url = str(rr.get('_photo_url', '')).strip()
                    if url:
                        coord_lookup[(la, lo)] = url
                except Exception:
                    continue
            if coord_lookup:
                def by_coord(r):
                    ...
                    try:
                        ...
                        return ''
                out['foto'] = out.apply(by_coord, axis=1)
...
def build_node_popup_html(row, header_color: str) -> str:
    ...
    if foto:
        ...
        if source_url:
            img_html += (
                f'<div style="font-size:10px;text-align:right;margin-bottom:4px;">'
                f'<a href="{html.escape(source_url)}" target="_blank" rel="noopener noreferrer" '
                'style="color:#2e7d32;text-decoration:none;">📷 Sumber foto Google Sheets</a></div>'
            )
    elif source_url:
        img_html = (
            ...
            '📷 Foto tersedia pada sumber Google Sheets<br>'
            f'<a href="{html.escape(source_url)}" target="_blank" rel="noopener noreferrer" '
            'style="color:#2e7d32;font-weight:700;text-decoration:none;">Buka sumber foto</a>'
            '</div>'
        )
    else:
        img_html = ''
...
def load_all_corridors():
    ...
    for gid, meta in CORRIDOR_SHEETS.items():
        ...
        if 'foto_source' not in d.columns:
            d['foto_source'] = PHOTO_SOURCES.get(gid, {}).get('url', '')
        else:
            d['foto_source'] = d['foto_source'].fillna(
                PHOTO_SOURCES.get(gid, {}).get('url', '')
            )
        frames.append(d)
...
def policy_brief_docx_bytes(corridor_df, indicator_df, selected_scope):
    """Membangun versi editable Policy Brief UVI dalam format Word (.docx),
    berisi konten yang sama dengan policy_brief_markdown() namun disusun
    langsung sebagai elemen dokumen Word (heading, paragraf, list) sehingga
    hasilnya siap diedit di Microsoft Word / Google Docs."""
    ...
    title = doc.add_heading(
        "POLICY BRIEF — Perancangan Kota Berbasis Urban Visual Index (UVI)",
        level=0,
    )
    title.alignment = WD_ALIGN_PARAGRAPH.LEFT

    scope_p = doc.add_paragraph()
    scope_run = scope_p.add_run(f"Wilayah analisis: {selected_scope}")
    scope_run.bold = True
    ...
    def add_bold_bullet(prefix, bold_part, suffix):
        p = doc.add_paragraph(style="List Bullet")
        p.add_run(prefix)
        p.add_run(bold_part).bold = True
        p.add_run(suffix)

    def add_numbered_bold(prefix_bold, rest):
    ...
    add_body(
        ...
        "tingkat koridor dan node. Koridor dengan UVI rendah perlu diprioritaskan untuk "
        "intervensi desain, sedangkan koridor dengan UVI tinggi diarahkan untuk pemeliharaan "
        "kualitas dan perlindungan karakter ruang."
    )

    doc.add_heading("2. Temuan utama", level=1)
    add_bold_bullet(
        "Koridor dengan prioritas intervensi tertinggi: ",
        top["Koridor"],
        f" (UVI {top['Rata-rata UVI']:.2f}).",
    )
    add_bold_bullet(
        "Koridor dengan kualitas visual tertinggi dalam cakupan analisis: ",
        strongest["Koridor"],
        f" (UVI {strongest['Rata-rata UVI']:.2f}).",
    )
    add_bold_bullet(
        "Indikator terendah: ", weakest["Indikator"], f" ({weakest['Rata-rata']:.3f})."
    )
    add_bold_bullet(
        "Koridor UVI rendah (<5): ", str(high), f"; koridor UVI sedang (5–<7): {medium}."
    )

    doc.add_heading("3. Arahan perancangan kota", level=1)
    add_numbered_bold(
        "Prioritaskan node UVI rendah ",
        "sebagai lokasi pilot improvement, bukan melakukan intervensi seragam pada seluruh koridor.",
    )
    add_numbered_bold(
        "Gunakan indikator terendah sebagai diagnosis desain",
        ": vegetasi → canopy/green buffer; aksesibilitas → trotoar dan crossing; aktivitas → "
        "active frontage dan seating; kendaraan → traffic calming/parkir; heritage → konservasi "
        "karakter visual.",
    )
    add_numbered_bold(
        "Terapkan pendekatan complete street dan universal design ",
        "pada ruang jalan yang menjadi penghubung ruang publik.",
    )
    add_numbered_bold(
        "Pertahankan koridor UVI tinggi ",
        "melalui design guideline, pemeliharaan vegetasi, street furniture, fasad, vista, dan "
        "pengendalian elemen visual yang berpotensi menurunkan kualitas.",
    )
    add_numbered_bold(
        "Gunakan simulasi UVI sebagai alat ex-ante ",
        "untuk membandingkan alternatif desain sebelum implementasi fisik.",
    )

    doc.add_heading("4. Tata kelola implementasi", level=1)
    add_body(
        "UVI dapat digunakan sebagai dashboard monitoring lintas perangkat daerah untuk "
        "menetapkan lokasi prioritas, menyusun desain teknis, mengalokasikan anggaran, dan "
        "mengevaluasi perubahan kualitas ruang setelah intervensi."
    )

    doc.add_heading("5. Catatan metodologis", level=1)
    p = doc.add_paragraph()
    p.add_run("UVI merupakan indeks komposit berbasis 8 indikator visual. Rekomendasi ini adalah ")
    p.add_run("diagnosis berbasis indikator").bold = True
    p.add_run(
        ", sehingga keputusan desain akhir tetap memerlukan verifikasi lapangan, aspek "
        "keselamatan, aksesibilitas, utilitas, regulasi, dan partisipasi pengguna ruang."
    )

    footer = doc.add_paragraph()
    footer_run = footer.add_run(
        f"\nDokumen dihasilkan otomatis oleh UVIP Malang pada {datetime.now():%d %B %Y}."
    )
...
""")
        ...
        with st.expander("Versi editable (Word / .docx)", expanded=False):
            pb_docx = policy_brief_docx_bytes(corridor_pb, indicator_pb, pb_scope)
