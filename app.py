...
# Inisialisasi state pilihan koridor sekali saja (default: semua terpilih).
# Widget multiselect diberi `key` eksplisit dan TIDAK memakai parameter
# `default` — pola ini mencegah pilihan majemuk ter-reset ke satu opsi saat
# widget lain di sidebar (mis. slider bobot) memicu rerun skrip.
if "selected_corridors" not in st.session_state:
    st.session_state.selected_corridors = corridor_names.copy()
else:
    # Buang pilihan lama yang sudah tidak ada di data (mis. ganti sumber data)
    st.session_state.selected_corridors = [
        c for c in st.session_state.selected_corridors if c in corridor_names
    ] or corridor_names.copy()

btn_all, btn_none = st.sidebar.columns(2)
if btn_all.button("☑️ Pilih semua", use_container_width=True):
    st.session_state.selected_corridors = corridor_names.copy()
    st.rerun()
if btn_none.button("⬜ Kosongkan", use_container_width=True):
    st.session_state.selected_corridors = []
    st.rerun()
